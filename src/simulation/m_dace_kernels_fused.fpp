...
module m_dace_kernels_fused
  ...
  implicit none
  private
  public :: s_dace_fused_x, dace_fused_dirs, dace_fused_contract
...
#define MFC_DACE_BAKED_BUFF 2
  ...
  interface
    subroutine fused_run_x(state, &
          ...
          & vsrc_d1, &
          & vsrc_d2) &
        bind(C, name='__program_mfc_dace_fused_x')
    ...
    function fused_init_x(&
          ...
          & vsrc_d1, &
          & vsrc_d2) &
        bind(C, name='__dace_init_mfc_dace_fused_x')
      import :: c_ptr, c_int, c_int64_t, c_double
      type(c_ptr) :: fused_init_x
      integer(c_int64_t), value :: d_cbl_x_d0
      integer(c_int64_t), value :: d_cbr_x_d0
      integer(c_int64_t), value :: flux_d0
      integer(c_int64_t), value :: flux_d1
    ...
    function fused_exit_x(state) bind(C, name='__dace_exit_mfc_dace_fused_x')
      import :: c_ptr, c_int
      integer(c_int) :: fused_exit_x
      type(c_ptr), value :: state
    end function
  end interface
  interface
    subroutine fused_run_y(state, &
          & accel_bf, &
          & acoustic_source, &
          & adap_dt, &
          & adap_dt_max_iters, &
          ...
          & vsrc_d1, &
          & vsrc_d2) &
        bind(C, name='__program_mfc_dace_fused_y')
      import :: c_ptr, c_int, c_int64_t, c_double
      type(c_ptr), value :: state
      type(c_ptr), value :: accel_bf
      type(c_ptr), value :: acoustic_source
    ...
    function fused_init_y(&
          ...
          & vsrc_d1, &
          & vsrc_d2) &
        bind(C, name='__dace_init_mfc_dace_fused_y')
      import :: c_ptr, c_int, c_int64_t, c_double
      type(c_ptr) :: fused_init_y
      integer(c_int64_t), value :: d_cbl_y_d0
      integer(c_int64_t), value :: d_cbr_y_d0
      integer(c_int64_t), value :: flux_d0
      integer(c_int64_t), value :: flux_d1
    ...
    function fused_exit_y(state) bind(C, name='__dace_exit_mfc_dace_fused_y')
      import :: c_ptr, c_int
      integer(c_int) :: fused_exit_y
      type(c_ptr), value :: state
    end function
  end interface
  interface
    subroutine fused_run_z(state, &
          & accel_bf, &
          & acoustic_source, &
          & adap_dt, &
          & adap_dt_max_iters, &
          ...
          & vsrc_d1, &
          & vsrc_d2) &
        bind(C, name='__program_mfc_dace_fused_z')
      import :: c_ptr, c_int, c_int64_t, c_double
      type(c_ptr), value :: state
      type(c_ptr), value :: accel_bf
      type(c_ptr), value :: acoustic_source
    ...
    function fused_init_z(&
          ...
          & vsrc_d1, &
          & vsrc_d2) &
        bind(C, name='__dace_init_mfc_dace_fused_z')
      import :: c_ptr, c_int, c_int64_t, c_double
      type(c_ptr) :: fused_init_z
      integer(c_int64_t), value :: d_cbl_z_d0
      integer(c_int64_t), value :: d_cbr_z_d0
      integer(c_int64_t), value :: flux_d0
      integer(c_int64_t), value :: flux_d1
    ...
    function fused_exit_z(state) bind(C, name='__dace_exit_mfc_dace_fused_z')
      import :: c_ptr, c_int
      integer(c_int) :: fused_exit_z
      type(c_ptr), value :: state
    end function
  end interface
...
contains
  ...
  function dace_fused_contract() result(c)
    logical :: c
    c = (model_eqns == model_eqns_5eq .and. num_fluids == 2 .and. num_dims == 3 .and. &
         ...
         & weno_order == 5 .and. weno_num_stencils == 2 .and. &
         & .not. mapped_weno .and. .not. wenoz .and. .not. teno .and. .not. mp_weno .and. &
  ...
  end function dace_fused_contract
  ...
  subroutine s_dace_fused_x(v_rs, pcL, pcR, dL, dR, flux_rsx, fsrc_rsx, vsrc_rsx, &
    ...
    type(c_ptr), save :: state_fused(3) = [c_null_ptr, c_null_ptr, c_null_ptr]
    integer, save :: state_ext_fused(3) = [-1, -1, -1]
    ...
    if (.not. fused_announced) then
      print *, 'm_dace_kernels_fused: FUSED sweep dispatch active (x,y,z)=', &
               dace_fused_dirs(1), dace_fused_dirs(2), dace_fused_dirs(3)
    ...
    if (state_ext_fused(dir_in) /= ext) then
      if (c_associated(state_fused(dir_in))) then
        select case (dir_in)
        case (2)
          ierr = fused_exit_y(state_fused(2))
        case (3)
          ierr = fused_exit_z(state_fused(3))
        case default
          ierr = fused_exit_x(state_fused(1))
        end select
      end if
      select case (dir_in)
      case (2)
      state_fused(2) = fused_init_y(&
      ...
      state_ext_fused(dir_in) = ext
    end if

    select case (dir_in)
    case (2)
      call fused_run_y(state_fused(2), &
          ...
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, dl_dev, dr_dev, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_flux, d_fsrc, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_gammas, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          ...
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_pi_infs, pcl_dev, pcr_dev, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & re_dev, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_qvs, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & ridx_dev, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, rgs_dev, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, vrs_dev, &
          & d_zeros, d_zeros, d_vsrc, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, int(d0d, c_int64_t), int(d0d, c_int64_t), nv64, &
          & e64, e64, nv64, e64, e64, int(jd(1), c_int), &
          & int(jd(2), c_int), int(jd(3), c_int), int(jd(4), c_int), int(jd(5), c_int), int(jd(6), c_int), 1_c_int64_t, &
          & 1_c_int64_t, 1_c_int64_t, 1_c_int64_t, 1_c_int64_t, int(d0pc, c_int64_t), int(d1pc, c_int64_t), &
          & int(d0pc, c_int64_t), int(d1pc, c_int64_t), re_e64, re_e64, re_e64, 2_c_int64_t, &
          & 2_c_int64_t, int(rsz1, c_int), int(rsz2, c_int), e64, e64, e64, &
          & int(nvels_l, c_int64_t), e64, e64)
    case (3)
      call fused_run_z(state_fused(3), &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          ...
          & int(nvels_l, c_int64_t), e64, e64)
    case default
      call fused_run_x(state_fused(1), &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
          & d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, d_zeros, &
...
end module m_dace_kernels_fused
#endif
